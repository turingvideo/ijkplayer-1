...
void pop_av_log(FFPlayer *ffp, void *avcl, int level, const char *fmt, ...);
...
static void control_video_queue_duration(FFPlayer *ffp, VideoState *is) {
    ...
    if (cached_duration > is->max_cached_duration) {
        // drop
        pop_av_log(ffp, NULL, AV_LOG_INFO, "233 video cached_duration = %ld, nb_packets = %d.\n",
                   cached_duration, nb_packets);
        drop_to_pts = is->videoq.last_pkt->pkt.pts;  // 这里删掉一半，你也可以自己修改，依据设置进来的max_cached_duration大小
...
static void control_audio_queue_duration(FFPlayer *ffp, VideoState *is) {
    ...
    if (cached_duration > is->max_cached_duration) {
        // drop
        pop_av_log(ffp, NULL, AV_LOG_INFO, "233 audio cached_duration = %ld, nb_packets = %d.\n",
                   cached_duration, nb_packets);
...
convert_image(FFPlayer *ffp, AVFrame *src_frame, int64_t src_frame_pts, int width, int height) {
    ...
    if ((int) (origin_dar * 100) != (int) (dar * 100)) {
        ...
        pop_av_log(ffp, NULL, AV_LOG_INFO, "%s img_info->width = %d, img_info->height = %d\n",
                   __func__,
    ...
    if (!img_info->frame_img_convert_ctx) {
        ...
        if (!img_info->frame_img_convert_ctx) {
            ret = -1;
            pop_av_log(ffp, NULL, AV_LOG_ERROR, "%s sws_getContext failed\n", __func__);
            goto fail0;
        }
    }
    ...
    if (!img_info->frame_img_codec_ctx) {
        ...
        if (!image_codec) {
            ret = -1;
            pop_av_log(ffp, NULL, AV_LOG_ERROR, "%s avcodec_find_encoder failed\n", __func__);
            goto fail0;
        }
        img_info->frame_img_codec_ctx = avcodec_alloc_context3(image_codec);
        if (!img_info->frame_img_codec_ctx) {
            ret = -1;
            pop_av_log(ffp, NULL, AV_LOG_ERROR, "%s avcodec_alloc_context3 failed\n", __func__);
            goto fail0;
        }
    ...
    if (!dst_frame) {
        ret = -1;
        pop_av_log(ffp, NULL, AV_LOG_ERROR, "%s av_frame_alloc failed\n", __func__);
        goto fail0;
    }
    ...
    if (!buffer) {
        ret = -1;
        pop_av_log(ffp, NULL, AV_LOG_ERROR, "%s av_image_get_buffer_size failed\n", __func__);
        goto fail1;
    }
    ...
    if (ret < 0) {
        ret = -1;
        pop_av_log(ffp, NULL, AV_LOG_ERROR, "%s av_image_fill_arrays failed\n", __func__);
        goto fail2;
    }
    ...
    if (ret <= 0) {
        ret = -1;
        pop_av_log(ffp, NULL, AV_LOG_ERROR, "%s sws_scale failed\n", __func__);
        goto fail2;
    }
    ...
    if (ret >= 0 && got_packet > 0) {
        ...
        if (fd < 0) {
            ret = -1;
            pop_av_log(ffp, NULL, AV_LOG_ERROR, "%s open path = %s failed %s\n", __func__,
                       file_path,
                       strerror(errno));
            goto fail2;
        }
        ...
        if (pkt.data == flush_pkt.data) {
            ...
            if (d->avctx->codec_type == AVMEDIA_TYPE_SUBTITLE) {
                ...
                if (avcodec_send_packet(d->avctx, &pkt) == AVERROR(EAGAIN)) {
                    pop_av_log(ffp, d->avctx, AV_LOG_ERROR,
...
static void stream_close(FFPlayer *ffp) {
    ...
    pop_av_log(ffp, NULL, AV_LOG_DEBUG, "wait for read_tid\n");
    ...
    pop_av_log(ffp, NULL, AV_LOG_DEBUG, "wait for video_refresh_tid\n");
...
static double compute_target_delay(FFPlayer *ffp, double delay, VideoState *is) {
    ...
    pop_av_log(ffp, NULL, AV_LOG_TRACE, "video: delay=%0.3f A-V=%f\n",
            delay, -diff);
#endif
...
static void video_refresh(FFPlayer *opaque, double *remaining_time) {
    ...
    if (ffp->show_status) {
        ...
        if (!last_time || (cur_time - last_time) >= 30000) {
            ...
            pop_av_log(ffp, NULL, AV_LOG_INFO,
...
static void alloc_picture(FFPlayer *ffp, int frame_format) {
    ...
    if (!vp->bmp || (!vp->bmp->is_private && vp->bmp->pitches[0] < vp->width)) {
        ...
        pop_av_log(ffp, NULL, AV_LOG_FATAL,
...
queue_picture(FFPlayer *ffp, AVFrame *src_frame, double pts, double duration, int64_t pos,
        ...
        if (!isnan(pts)) {
            ...
            if ((pts * 1000 * 1000 < is->seek_pos) || deviation > MAX_DEVIATION) {
                ...
                if (is->drop_vframe_count == 0) {
                    ...
                    pop_av_log(ffp, NULL, AV_LOG_INFO,
                               "video accurate_seek start, is->seek_pos=%" PRId64 ", pts=%lf, is->accurate_seek_time = %" PRId64 "\n",
                ...
                if ((now - is->accurate_seek_start_time) <= ffp->accurate_seek_timeout) {
                    ...
                    pop_av_log(ffp, NULL, AV_LOG_WARNING,
                               "video accurate_seek is error, is->drop_vframe_count=%d, now = %" PRId64 ", pts = %lf\n",
                               is->drop_vframe_count, now, pts);
                    video_accurate_seek_fail = 1;  // if KEY_FRAME interval too big, disable accurate seek
                }
            } else {
                pop_av_log(ffp, NULL, AV_LOG_INFO,
                           "video accurate_seek is ok, is->drop_vframe_count =%d, is->seek_pos=%" PRId64 ", pts=%lf\n",
    ...
    if (vp->bmp) {
        ...
        if (SDL_VoutFillFrameYUVOverlay(vp->bmp, src_frame) < 0) {
            pop_av_log(ffp, NULL, AV_LOG_FATAL, "Cannot initialize the conversion context\n");
            exit(1);
        }
...
#define INSERT_FILT(name, arg) do {                                          \
    ...
    if (fabsf(ffp->pf_playback_rate) > 0.00001 &&
        ...
        pop_av_log(ffp, ffp, AV_LOG_INFO, "vf_rate=%f(1/%f)\n", ffp->pf_playback_rate, rate);
...
static int configure_audio_filters(FFPlayer *ffp, const char *afilters, int force_output_format)
{
    ...
    if (fabsf(ffp->pf_playback_rate) > 0.00001 &&
        ...
        pop_av_log(ffp, ffp, AV_LOG_INFO, "af_rate=%f\n", ffp->pf_playback_rate);
        ...
        if (got_frame) {
            ...
            if (ffp->enable_accurate_seek && is->audio_accurate_seek_req && !is->seek_req) {
                ...
                if (!isnan(frame_pts)) {
                    ...
                    if ((audio_clock * 1000 * 1000 < is->seek_pos) || deviation > MAX_DEVIATION) {
                        if (is->drop_aframe_count == 0) {
                            ...
                            SDL_UnlockMutex(is->accurate_seek_mutex);
                            pop_av_log(ffp, NULL, AV_LOG_INFO,
                                       "audio accurate_seek start, is->seek_pos=%" PRId64 ", audio_clock=%lf, is->accurate_seek_start_time = %" PRId64 "\n",
                ...
                if (audio_accurate_seek_fail) {
                    pop_av_log(ffp, NULL, AV_LOG_INFO,
                               "audio accurate_seek is error, is->drop_aframe_count=%d, now = %" PRId64 ", audio_clock = %lf\n",
            ...
            if (reconfigure) {
                ...
                pop_av_log(ffp, NULL, AV_LOG_DEBUG,
...
static int ffplay_video_thread(void *arg) {
        ...
        if (ffp->get_frame_mode) {
            ...
            if (pts >= dst_pts) {
                while (retry_convert_image <= MAX_RETRY_CONVERT_IMAGE) {
                    ...
                    retry_convert_image++;
                    pop_av_log(ffp, NULL, AV_LOG_ERROR,
                ...
                if (ret || ffp->get_img_info->count <= 0) {
                    if (ret) {
                        pop_av_log(ffp, NULL, AV_LOG_ERROR, "convert image abort ret = %d\n", ret);
    ...
    avfilter_graph_free(&graph);
#endif
    pop_av_log(ffp, NULL, AV_LOG_INFO, "convert image convert_frame_count = %d\n",
               convert_frame_count);
    av_frame_free(&frame);
    return 0;
}
...
static int audio_decode_frame(FFPlayer *ffp) {
    ...
    if (is->swr_ctx) {
        ...
        int len2;
        if (out_size < 0) {
            pop_av_log(ffp, NULL, AV_LOG_ERROR, "av_samples_get_buffer_size() failed\n");
            return -1;
        }
        if (wanted_nb_samples != af->frame->nb_samples) {
            if (swr_set_compensation(is->swr_ctx, (wanted_nb_samples - af->frame->nb_samples) *
                ...
                pop_av_log(ffp, NULL, AV_LOG_ERROR, "swr_set_compensation() failed\n");
                return -1;
            }
        }
        ...
        if (len2 < 0) {
            pop_av_log(ffp, NULL, AV_LOG_ERROR, "swr_convert() failed\n");
            return -1;
        }
        if (len2 == out_count) {
            pop_av_log(ffp, NULL, AV_LOG_WARNING, "audio buffer is probably too small\n");
            if (swr_init(is->swr_ctx) < 0)
                swr_free(&is->swr_ctx);
        }
...
static int audio_open(FFPlayer *opaque, int64_t wanted_channel_layout, int wanted_nb_channels,
    ...
    if (wanted_spec.freq <= 0 || wanted_spec.channels <= 0) {
        pop_av_log(ffp, NULL, AV_LOG_ERROR, "Invalid sample rate or channel count!\n");
        return -1;
    }
    ...
    while (SDL_AoutOpenAudio(ffp->aout, &wanted_spec, &spec) < 0) {
        ...
        if (is->abort_request)
            return -1;
        pop_av_log(ffp, NULL, AV_LOG_WARNING, "SDL_OpenAudio (%d channels, %d Hz): %s\n",
...
static int stream_component_open(FFPlayer *ffp, int stream_index) {
    ...
    if (stream_lowres > av_codec_get_max_lowres(codec)) {
        pop_av_log(ffp, avctx, AV_LOG_WARNING,
    ...
    if ((t = av_dict_get(opts, "", NULL, AV_DICT_IGNORE_SUFFIX))) {
        pop_av_log(ffp, NULL, AV_LOG_ERROR, "Option %s not found.\n", t->key);
#ifdef FFP_MERGE
            ...
            if (ffp->max_fps >= 0) {
                if (is->video_st->avg_frame_rate.den && is->video_st->avg_frame_rate.num) {
                    ...
                    if (fps > ffp->max_fps && fps < 130.0) {
                        ...
                        pop_av_log(ffp, ffp, AV_LOG_WARNING, "fps: %lf (too high)\n", fps);
                    } else {
                        pop_av_log(ffp, ffp, AV_LOG_WARNING, "fps: %lf (normal)\n", fps);
                    }
                }
...
static int read_thread(void *arg) {
    ...
    if (!wait_mutex) {
        pop_av_log(ffp, NULL, AV_LOG_FATAL, "SDL_CreateMutex(): %s\n", SDL_GetError());
        ret = AVERROR(ENOMEM);
        goto fail;
    }
    ...
    if (!ic) {
        pop_av_log(ffp, NULL, AV_LOG_FATAL, "Could not allocate context.\n");
        ret = AVERROR(ENOMEM);
        goto fail;
    }
    ...
    if (av_stristart(is->filename, "rtmp", NULL) ||
        ...
        pop_av_log(ffp, ffp, AV_LOG_WARNING, "remove 'timeout' option for rtmp.\n");
    ...
    pop_av_log(ffp, ffp, AV_LOG_INFO, "max_frame_duration: %.3f\n", is->max_frame_duration);
    ...
    if (ffp->start_time != AV_NOPTS_VALUE) {
        ...
        if (ret < 0) {
            pop_av_log(ffp, NULL, AV_LOG_WARNING, "%s: could not seek to position %0.3f\n",
...
// END   =========== Add control cache queue ============
    ...
    if (is->video_stream < 0 && is->audio_stream < 0) {
        pop_av_log(ffp, NULL, AV_LOG_FATAL, "Failed to open file '%s' or configure filtergraph\n",
                   is->filename);
        ret = -1;
...
//      of the seek_pos/seek_rel variables
        ...
        if ((!is->paused || completed) &&
            ...
            if (ffp->loop != 1 && (!ffp->loop || --ffp->loop)) {
                ...
                ffp_statistic_l(ffp);
                if (completed) {
                    pop_av_log(ffp, ffp, AV_LOG_INFO, "ffp_toggle_buffering: eof\n");
                    SDL_LockMutex(wait_mutex);
                    ...
                    toggle_pause(ffp, 1);
                    if (ffp->error) {
                        pop_av_log(ffp, ffp, AV_LOG_INFO, "ffp_toggle_buffering: error: %d\n",
                                   ffp->error);
                        ...
                        ALOGW("msg complete, duration %ld. position %ld", ffp_get_duration_l(ffp),
                              ffp_get_current_position_l(ffp));
                        pop_av_log(ffp, ffp, AV_LOG_INFO, "ffp_toggle_buffering: completed: OK\n");
        ...
        if (ret < 0) {
            ...
            if (pb_error) {
                ...
                is->eof = 1;
                ffp->error = pb_error;
                pop_av_log(ffp, ffp, AV_LOG_ERROR, "av_read_frame error: %s\n",
...
static VideoState *stream_open(FFPlayer *ffp, const char *filename, AVInputFormat *iformat) {
    ...
    if (!(is->continue_read_thread = SDL_CreateCond())) {
        pop_av_log(ffp, NULL, AV_LOG_FATAL, "SDL_CreateCond(): %s\n", SDL_GetError());
        goto fail;
    }
    ...
    if (ffp->startup_volume < 0)
        pop_av_log(ffp, NULL, AV_LOG_WARNING, "-volume=%d < 0, setting to 0\n",
                   ffp->startup_volume);
    if (ffp->startup_volume > 100)
        pop_av_log(ffp, NULL, AV_LOG_WARNING, "-volume=%d > 100, setting to 100\n",
                   ffp->startup_volume);
    ...
    if (!is->read_tid) {
        pop_av_log(ffp, NULL, AV_LOG_FATAL, "SDL_CreateThread(): %s\n", SDL_GetError());
        goto fail;
    }
...
FFPlayer *ffp_create() {
    ...
    pop_av_log(ffp, NULL, AV_LOG_INFO, "av_version_info: %s\n", av_version_info());
    pop_av_log(ffp, NULL, AV_LOG_INFO, "ijk_version_info: %s\n", ijk_version_info());
...
void ffp_destroy(FFPlayer *ffp) {
    ...
    if (ffp->is) {
        pop_av_log(ffp, NULL, AV_LOG_WARNING, "ffp_destroy_ffplayer: force stream_close()");
        stream_close(ffp);
        ffp->is = NULL;
    }
...
static AVDictionary **ffp_get_opt_dict(FFPlayer *ffp, int opt_category) {
    ...
    switch (opt_category) {
        ...
        case FFP_OPT_CATEGORY_SWR:
            return &ffp->swr_opts;
        default:
            pop_av_log(ffp, ffp, AV_LOG_ERROR, "unknown option category %d\n", opt_category);
            return NULL;
    }
}
...
#endif
        default:
            pop_av_log(ffp, ffp, AV_LOG_ERROR,
...
static void ffp_show_dict(FFPlayer *ffp, const char *tag, AVDictionary *dict) {
    ...
    while ((t = av_dict_get(dict, "", t, AV_DICT_IGNORE_SUFFIX))) {
        pop_av_log(ffp, ffp, AV_LOG_INFO, "%-*s: %-*s = %s\n", 12, tag, 28, t->key, t->value);
    }
}
...
int ffp_prepare_async_l(FFPlayer *ffp, const char *file_name) {
    ...
    if (strlen(file_name) + 1 > 1024) {
        pop_av_log(ffp, ffp, AV_LOG_ERROR, "%s too long url\n", __func__);
    ...
    pop_av_log(ffp, NULL, AV_LOG_INFO, "===== versions =====\n");
    ...
    pop_av_log(ffp, NULL, AV_LOG_INFO, "===== options =====\n");
    ...
    if (!is) {
        pop_av_log(ffp, NULL, AV_LOG_WARNING, "ffp_prepare_async_l: stream_open failed OOM");
...
int ffp_seek_to_l(FFPlayer *ffp, long msec) {
    ...
    // FIXME: 9 seekable
    pop_av_log(ffp, ffp, AV_LOG_DEBUG, "stream_seek %"PRId64"(%d) + %"PRId64", \n", seek_pos,
               (int) msec,
               start_time);
...
void ffp_toggle_buffering_l(FFPlayer *ffp, int buffering_on) {
    ...
    if (buffering_on && !is->buffering_on) {
        pop_av_log(ffp, ffp, AV_LOG_DEBUG, "ffp_toggle_buffering_l: start\n");
        is->buffering_on = 1;
        stream_update_pause_l(ffp);
        if (is->seek_req) {
            is->seek_buffering = 1;
        ...
        pop_av_log(ffp, ffp, AV_LOG_DEBUG, "ffp_toggle_buffering_l: end\n");
        is->buffering_on = 0;
        stream_update_pause_l(ffp);
...
void ffp_check_buffering_l(FFPlayer *ffp) {
    ...
    if (hwm_in_ms > 0) {
        ...
        if (is->audio_st && audio_time_base_valid) {
            ...
            pop_av_log(ffp, ffp, AV_LOG_DEBUG, "audio cache=%%%d milli:(%d/%d) bytes:(%d/%d) packet:(%d/%d)\n", audio_cached_percent,
        ...
        if (is->video_st && video_time_base_valid) {
            ...
            pop_av_log(ffp, ffp, AV_LOG_DEBUG, "video cache=%%%d milli:(%d/%d) bytes:(%d/%d) packet:(%d/%d)\n", video_cached_percent,
...
#ifdef FFP_SHOW_DEMUX_CACHE
            pop_av_log(ffp, ffp, AV_LOG_DEBUG, "time cache=%%%d (%d/%d)\n", buf_time_percent, cached_duration_in_ms, hwm_in_ms);
#endif
...
#ifdef FFP_SHOW_DEMUX_CACHE
        pop_av_log(ffp, ffp, AV_LOG_DEBUG, "size cache=%%%d (%d/%d)\n", buf_size_percent, cached_size, hwm_in_bytes);
#endif
    ...
    if (buf_percent) {
        ...
        pop_av_log(ffp, ffp, AV_LOG_DEBUG, "buf pos=%"PRId64", %%%d\n", buf_time_position, buf_percent);
#endif
...
void ffp_set_video_codec_info(FFPlayer *ffp, const char *module, const char *codec) {
    ...
    pop_av_log(ffp, ffp, AV_LOG_INFO, "VideoCodec: %s\n", ffp->video_codec_info);
}
...
void ffp_set_audio_codec_info(FFPlayer *ffp, const char *module, const char *codec) {
    ...
    pop_av_log(ffp, ffp, AV_LOG_INFO, "AudioCodec: %s\n", ffp->audio_codec_info);
}
...
void ffp_set_subtitle_codec_info(FFPlayer *ffp, const char *module, const char *codec) {
    ...
    pop_av_log(ffp, ffp, AV_LOG_INFO, "SubtitleCodec: %s\n", ffp->subtitle_codec_info);
}
...
void ffp_set_playback_rate(FFPlayer *ffp, float rate) {
    ...
    pop_av_log(ffp, ffp, AV_LOG_INFO, "Playback rate: %f\n", rate);
    ffp->pf_playback_rate = rate;
...
int ffp_set_stream_selected(FFPlayer *ffp, int stream, int selected) {
    ...
    if (stream < 0 || stream >= ic->nb_streams) {
        pop_av_log(ffp, ffp, AV_LOG_ERROR, "invalid stream index %d >= stream number (%d)\n",
                   stream,
                   ic->nb_streams);
        return -1;
    }
    ...
    if (selected) {
        switch (codecpar->codec_type) {
            ...
            case AVMEDIA_TYPE_SUBTITLE:
                ...
                break;
            default:
                pop_av_log(ffp, ffp, AV_LOG_ERROR, "select invalid stream %d of video type %d\n",
                           stream,
                           codecpar->codec_type);
                return -1;
        }
        ...
        switch (codecpar->codec_type) {
            ...
            case AVMEDIA_TYPE_SUBTITLE:
                ...
                break;
            default:
                pop_av_log(ffp, ffp, AV_LOG_ERROR, "select invalid stream %d of audio type %d\n",
                           stream,
                           codecpar->codec_type);
                return -1;
        }
...
void formatString(char *buffer, size_t bufsize, const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);

    // 使用 vsnprintf 将可变参数格式化为字符串
